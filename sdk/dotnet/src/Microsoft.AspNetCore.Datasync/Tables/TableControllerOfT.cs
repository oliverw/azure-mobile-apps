...
namespace Microsoft.AspNetCore.Datasync
{
    ...
    public class TableController<TEntity, TVersion> : ControllerBase where TEntity : class, ITableData<TVersion>
    {
        /// <summary>
        ...
        public TableController(IRepository<TEntity, TVersion> repository = null, IAccessControlProvider<TEntity, TVersion> accessControlProvider = null, TableControllerOptions options = null)
        {
            _repository = repository;
            AccessControlProvider = accessControlProvider ?? new AccessControlProvider<TEntity, TVersion>();
        ...
        private IRepository<TEntity, TVersion> _repository;

        /// <summary>
        /// The <see cref="IRepository{TEntity}"/> to use for data store operations.
        /// </summary>
        public IRepository<TEntity, TVersion> Repository
        {
        ...
        /// </summary>
        public IAccessControlProvider<TEntity, TVersion> AccessControlProvider { get; set; }
        #endregion
        ...
        public virtual async Task<IActionResult> DeleteAsync([FromRoute] string id, CancellationToken token = default)
        {
            ...
            Request.ParseConditionalRequest<TEntity, TVersion>(entity, out TVersion version);
        ...
        public virtual async Task<IActionResult> PatchAsync([FromRoute] string id, CancellationToken token = default)
        {
            ...
            if (patchDocument.ModifiesSystemProperties<TEntity, TVersion>(entity, out Dictionary<string, string[]> systemPropertyValidationErrors))
            {
            ...
            if (Options.EnableSoftDelete && entity.Deleted && !patchDocument.Contains<TEntity, TVersion>("replace", "/deleted", false))
            {
                Logger?.LogWarning("Patch({Id}): Item not found (soft-delete)", id);
                return StatusCode(StatusCodes.Status410Gone);
            }
            Request.ParseConditionalRequest(entity, out TVersion version);
        ...
        public virtual async Task<IActionResult> QueryAsync(CancellationToken token = default)
        {
            ...
            dataset = dataset
                .ApplyDataView<TEntity, TVersion>(AccessControlProvider.GetDataView())
                .ApplyDeletedView<TEntity, TVersion>(Request, Options.EnableSoftDelete);
        ...
        public virtual async Task<IActionResult> ReadAsync([FromRoute] string id, CancellationToken token = default)
        {
            ...
            Request.ParseConditionalRequest(entity, out TVersion _);
